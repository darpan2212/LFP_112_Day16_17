...
public class BinarySearchTree<K extends Comparable<K>> {
	...
	private MyBinaryNode<K> addRecursively(
		...
		if (compareResult == 0)
			return current;
		if (compareResult > 0) {
			current.left = this.addRecursively(current.left,
					key);
		} else {
			current.right = this
					.addRecursively(current.right, key);
		}
		return current;
	}

	public boolean search(K key) {
		boolean isContain = searchRecursively(this.root,
				key);
		return isContain;
	}
